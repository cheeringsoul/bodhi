...
@Slf4j
@CleanerMeta(chatId = -1001825096074L)
public class FollowInCleaner implements Cleaner<ChannelNewsEntity> {
    @Override
    public Optional<ChannelNewsEntity> clean(ChannelNewsEntity entity) {
        for (LinkContent item: entity.linkContents()) {
            ...
            String text = item.crawledData().doc().select("#article-content").text().trim();
            if (text.isEmpty()) {
                log.warn("[Ignore message] FollowInCleaner content is empty url {}", item.url());
                continue;
            }
            item.content(text);
        }
