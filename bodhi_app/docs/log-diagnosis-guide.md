...
Bodhi can diagnose production issues from log snippets. When a user pastes error logs into an AI assistant, Bodhi
matches the log text against `@bodhi.log.*` patterns in the knowledge graph, identifies which function produced the log,
locates it within a flow, and provides full upstream/downstream context.
...
Add log patterns to your function annotations. The `{placeholder}` syntax tells Bodhi how to parse the log and extract
business variables.
...
> PaymentService.hold failed with a timeout for order 12345. This is step 3 in the create_order flow. Steps 1-2 (create
> order, deduct inventory) likely succeeded, but step 4 (publish order_created event) was never reached. Downstream
> consumers (payment-service, notification-service) will not be triggered. The order is likely stuck in PENDING state.
...
Bodhi matches all three lines, showing which steps succeeded and which failed — without needing access to the actual log
files.
...
At startup, the engine scans all inline `@bodhi.log.success` and `@bodhi.log.error` tags and compiles them into regex
patterns:
...
When `diagnose_log` is called, each line of the user's log text is tested against every pattern. Matches extract the
named variables (`orderId`, `reason`) and link back to the function, flow, and knowledge graph.

## Limitations

- Only functions with `@bodhi.log.*` tags are matched by pattern. Untagged functions fall back to function-name
  matching.
- The diagnosis is based on the knowledge graph's static structure — it shows what *should* happen, not what *did*happen
  at runtime.
